...
for dataset_target in dataset_names:
    ...
    # %%
    for percentage in [0.1, 0.3, 0.5, 0.7, 0.9, 1]:
        print(f"Percentage: {percentage}")
        subject_ids_train, subject_ids_val = dict(), dict()
        for dataset_name in dataset_sources:
            if dataset_name == dataset_target:
                continue
            n_subjects = int(percentage * len(subject_ids[dataset_name]))
            print(f"Dataset: {dataset_name}, n_subjects: {n_subjects}")
            subject_ids_dataset = rng.choice(subject_ids[dataset_name], n_subjects, replace=False)
        ...
        for epoch in range(n_epochs):
            ...
            if min_val_loss > np.mean(val_loss):
                ...
                if patience_counter > patience:
                    ...
                    break
        history_path = f"results_all/history/history_{dataset_target}_{tma}_{percentage}.pkl"
        df_history = pd.DataFrame(history)
        df_history.to_pickle(history_path)

        torch.save(best_model, f"results_all/models/models_{dataset_target}_{tma}_{percentage}.pt")


        # %%

        results = []
        results_path = f"results_all/percentage/results_{dataset_target}_{tma}_{percentage}.pkl"
